...
# ----------------------------------------------------
# Update Date : Fev/2/2016
# Author : JT Graveaud
# original code updated to use AES algorithm in order to store password encrypted
# ----------------------------------------------------
...
def encrypt(myInput, password, keyfile):
...
def decrypt(myInput, password):
    ...
    myInput = binascii.a2b_base64(myInput)
    outtext = ''
    ...
    while block != "":
        ...
        if len(myInput) == filesize:
            plaintext = plaintext[0:-(plaintext[-1])]

        # write ciphertext into outtext
        for c in plaintext:
            outtext = outtext + chr(c)

        # grab next block from input file
        block = getBlock(myInput[j*16:(j+1)*16])
        j = j + 1
    return stripped(outtext.strip())


def printUsage():
    print "./pysec.py -k <keyfile> [--enc | --dec]"
    sys.exit()
...
def main(argv):
    ...
    if os.path.isfile(curpath+'/pysec_salt.enc'):
      f = open(curpath+'/pysec_salt.enc', 'rU')
      password = f.readline()
      f.close()
    else:
      f = open(curpath+'/pysec_salt.enc', 'wb')
      password = ''.join(random.choice(ALPHABET) for i in range(160))
      f.write(password)
      f.close()
      
    # get the key file to decrypt
    if script_opt('k', argv):
      keyfile = script_opt('k', argv)

    # get the text to decrypt
    if script_opt('t', argv):
      inputtext = script_opt('t', argv)
   
    if script_opt('enc', argv):
      optswitch = '-enc'
    if script_opt('dec', argv):
      optswitch = '-dec'

    if optswitch is None:
        print "Missing: options for encryption (--enc) or decryption (--dec)"
        printUsage()
    # encrypt text per user instructions
    if optswitch == "-enc":
        inputtext=getpass.getpass("Password for "+keyfile+": ")
        encrypt(inputtext, password, keyfile)

    # decrypt file per user instructions
    elif optswitch == "-dec":
        try:
          f = open(curpath+'/'+keyfile, 'r')
          inputtext = f.readline().strip()
          f.close()
        except:
          print 'The keyfile "'+keyfile+'" is missing in the directory : ../common/'
          print 'Khe following command will generate the keyfile required'
          print '../common/pysec.py --enc -k '+keyfile
          exit()
